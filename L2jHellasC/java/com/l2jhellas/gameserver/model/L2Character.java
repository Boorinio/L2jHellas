...
public abstract class L2Character extends L2Object
{
	...
	public void doCast(L2Skill skill)
	{
        ...
        if (Config.MOD_GVE_ENABLE_FACTION && this instanceof L2PcInstance && this.getTarget() instanceof L2PcInstance && this != null && this.getTarget() != null)
        {
        	...
        	if ((skill.getSkillType() == L2SkillType.HEAL ||
    				...
    				skill.getSkillType() == L2SkillType.NEGATE ||
    				skill.getSkillType() == L2SkillType.HEAL_PERCENT ||
    				skill.getSkillType() == L2SkillType.UNPOISON ||
    				skill.getSkillType() == L2SkillType.UNBLEED ||
    				skill.getTargetType() == L2SkillTargetType.TARGET_SELF ||
    				skill.getTargetType() == L2SkillTargetType.TARGET_PET ||
	...
	*/
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Add the L2Effect to the L2Character _effects</li> <li>If this effect
	 * doesn't belong to a Stack Group, add its Funcs to the Calculator set of
	 * the L2Character (remove the old one if necessary)</li> <li>If this effect
	 * has higher priority in its Stack Group, add its Funcs to the Calculator
	 * set of the L2Character (remove previous stacked effect Funcs if
	 * necessary)</li> <li>If this effect has NOT higher priority in its Stack
	 * Group, set the effect to Not In Use</li> <li>Update active skills in
	...
	public final void addEffect(L2Effect newEffect)
	{
		...
		synchronized (_effects)
		{
			...
			if (getBuffCount() > Config.BUFFS_MAX_AMOUNT && !doesStack(tempskill) && ((tempskill.getSkillType() == L2SkillType.BUFF || tempskill.getSkillType() == L2SkillType.DEBUFF || tempskill.getSkillType() == L2SkillType.REFLECT || tempskill.getSkillType() == L2SkillType.HEAL_PERCENT || tempskill.getSkillType() == L2SkillType.MANAHEAL_PERCENT) && !(tempskill.getId() > 4360 && tempskill.getId() < 4367)))
	 ...
	 * (Stop L2Effect)</li> <li>If the L2Effect belongs to a not empty Stack
	 * Group, replace theses Funcs by next stacked effect Funcs</li> <li>Remove
	 * the L2Effect from _effects of the L2Character</li> <li>Update active
	 ...
	 * L2Character</li> <li>If Stun succeed, active the abnormal effect Stun
	 * flag, notify the L2Character AI and send Server->Client UserInfo/CharInfo
	 * packet</li> <li>If Stun NOT succeed, send a system message Failed to the
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Delete a specified/all (if effect=null) immobilization until attacked
	 * abnormal L2Effect from L2Character and update client magic icon</li> <li>
	 * Set the abnormal effect flag _muted to False</li> <li>Notify the
	 * L2Character AI</li> <li>Send Server->Client UserInfo/CharInfo packet</li><BR>
	 * <BR>
	 */
	 ...
	 * from L2Character and update client magic icone</li> <li>Set the abnormal
	 * effect flag _confused to False</li> <li>Notify the L2Character AI</li>
	 ...
	 * <BR>
	 * <li>Remove Func added by this effect from the L2Character Calculator
	 * (Stop L2Effect)</li> <li>Remove the L2Effect from _effects of the
	 * L2Character</li> <li>Update active skills in progress icones on player
	 * client</li><BR>
	 * <BR>
	 * 
	 * @param type
	 ...
	 * from L2Character and update client magic icone</li> <li>Set the abnormal
	 * effect flag _fake_death to False</li> <li>Notify the L2Character AI</li><BR>
	 * <BR>
	 */
	 ...
	 * L2Character and update client magic icone</li> <li>Set the abnormal
	 * effect flag _affraid to False</li> <li>Notify the L2Character AI</li> <li>
	 * Send Server->Client UserInfo/CharInfo packet</li><BR>
	 * <BR>
	 */
	 ...
	 * L2Character and update client magic icone</li> <li>Set the abnormal
	 * effect flag _muted to False</li> <li>Notify the L2Character AI</li> <li>
	 * Send Server->Client UserInfo/CharInfo packet</li><BR>
	 * <BR>
	 */
	 ...
	 * L2Character and update client magic icone</li> <li>Set the abnormal
	 * effect flag _rooted to False</li> <li>Notify the L2Character AI</li> <li>
	 * Send Server->Client UserInfo/CharInfo packet</li><BR>
	 * <BR>
	 */
	 ...
	 * L2Character and update client magic icone</li> <li>Set the abnormal
	 * effect flag _sleeping to False</li> <li>Notify the L2Character AI</li>
	 * <li>Send Server->Client UserInfo/CharInfo packet</li><BR>
	 * <BR>
	 */
	 ...
	 * L2Character and update client magic icone</li> <li>Set the abnormal
	 * effect flag _stuned to False</li> <li>Notify the L2Character AI</li> <li>
	 * Send Server->Client UserInfo/CharInfo packet</li><BR>
	 * <BR>
	 */
	 ...
	 * <BR>
	 * <li>L2NPCInstance</li> <li>L2PcInstance</li> <li>L2Summon</li> <li>
	 * L2DoorInstance</li><BR>
	 * <BR>
	 */
	 ...
	 * ticks between the start and the destination</li> <li>_xSpeedTicks,
	 * _ySpeedTicks : Speed in unit/ticks</li><BR>
	 * <BR>
	 */
	 ...
	 * NPC_STD_CALCULATOR in _calculators</li> <li>Add the Func object to
	 * _calculators</li><BR>
	 * <BR>
	 * 
	 * @param f
	 ...
	 * <B><U> Example of use </U> :</B><BR>
	 * <BR>
	 * <li>Equip an item from inventory</li> <li>Learn a new passive skill</li>
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Delete movement data of the L2Character</li> <li>Set the current
	 * position (x,y,z), its current L2WorldRegion if necessary and its heading</li>
	 * <li>Remove the L2Object object from _gmList** of GmListTable</li> <li>
	 * Remove object from _knownObjects and _knownPlayer* of all surrounding
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Set the _target of L2Character to L2Object</li> <li>If necessary, add
	 * L2Object to _knownObject of the L2Character</li> <li>If necessary, add
	 * L2Character to _KnownObject of the L2Object</li> <li>If object==null,
	 * cancel Attak or Cast</li><BR>
	 * <BR>
	 * <B><U> Overriden in </U> :</B><BR>
	 * <BR>
	 * <li>L2PcInstance : Remove the L2PcInstance from the old target
	 * _statusListener and add it to the new target if it was a L2Character</li><BR>
	 * <BR>
	 * 
	 * @param object
	 *        L2object to target
	 ...
	 * onIntentionInteract(L2Object)</li> <li>FollowTask</li><BR>
	 * <BR>
	 * 
	 * @param x
	...
	public boolean moveToNextRoutePoint()
	{
		...
		// Send a Server->Client packet CharMoveToLocation to the actor and all
	 ...
	 * @see com.l2jhellas.gameserver.model.L2Character.isInsideRadius(int x, int
	 *      y, int z, int radius, boolean checkZ, boolean strictCheck)
	 */
	 ...
	 * TARGET_IN_PEACEZONE a Server->Client packet ActionFailed</li> <li>If
	 * target is confused, send a Server->Client packet ActionFailed</li> <li>If
	 * L2Character is a L2ArtefactInstance, send a Server->Client packet
	 * ActionFailed</li> <li>Send a Server->Client packet MyTargetSelected to
	 ...
	 * <li>Consumme MP, HP and Item if necessary</li> <li>Send a Server->Client
	 * packet StatusUpdate with MP modification to the L2PcInstance</li> <li>
	 * Launch the magic skill in order to calculate its effects</li> <li>If the
	 * skill type is PDAM, notify the AI of the target with AI_INTENTION_ATTACK</li>
