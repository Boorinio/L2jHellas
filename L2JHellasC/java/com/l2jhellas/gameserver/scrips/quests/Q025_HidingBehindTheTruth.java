package com.l2jhellas.gameserver.scrips.quests;

import java.util.HashMap;

import com.l2jhellas.gameserver.ai.CtrlIntention;
import com.l2jhellas.gameserver.emum.player.ChatType;
import com.l2jhellas.gameserver.model.actor.L2Npc;
import com.l2jhellas.gameserver.model.actor.instance.L2PcInstance;
import com.l2jhellas.gameserver.model.actor.position.Location;
import com.l2jhellas.gameserver.model.quest.Quest;
import com.l2jhellas.gameserver.model.quest.QuestState;
import com.l2jhellas.gameserver.network.serverpackets.NpcSay;
import com.l2jhellas.util.Rnd;

public class Q025_HidingBehindTheTruth extends Quest
{
	private static final String qn = "Q025_HidingBehindTheTruth";

	private static final int HIGH_PRIEST_AGRIPEL = 31348;
	private static final int PRIEST_BENEDICT = 31349;
	private static final int MYSTERIOUS_WIZARD = 31522;
	private static final int TOMBSTONE = 31531;
	private static final int MAID_OF_LIDIA = 31532;
	private static final int BROKEN_BOOKSHELF2 = 31533;
	private static final int BROKEN_BOOKSHELF3 = 31534;
	private static final int BROKEN_BOOKSHELF4 = 31535;
	private static final int COFFIN = 31536;

	private static final int TRIOL_PAWN = 27218;

	private static final int MAP_FOREST_OF_THE_DEAD = 7063;
	private static final int CONTRACT = 7066;
	private static final int LIDAS_DRESS = 7155;
	private static final int TOTEM_DOLL2 = 7156;
	private static final int GEMSTONE_KEY = 7157;
	private static final int TOTEM_DOLL3 = 7158;

	private static final int NECKLACE_OF_BLESSING = 936;
	private static final int EARING_OF_BLESSING = 874;
	private static final int RING_OF_BLESSING = 905;

	private static final int MIN_LVL = 66;
	private static final HashMap<Integer, Location> TRIOL_PAWN_LOC = new HashMap<>();
	private static final Location COFFIN_LOC = new Location(60104, -35820, -681);
	
	public Q025_HidingBehindTheTruth()
	{
		super(25, qn, "Hiding Behind the Truth");
		
		setItemsIds(GEMSTONE_KEY, CONTRACT, TOTEM_DOLL3, TOTEM_DOLL2, LIDAS_DRESS);
		addStartNpc(PRIEST_BENEDICT);
		addTalkId(HIGH_PRIEST_AGRIPEL, PRIEST_BENEDICT, MYSTERIOUS_WIZARD, TOMBSTONE, MAID_OF_LIDIA, BROKEN_BOOKSHELF2, BROKEN_BOOKSHELF3, BROKEN_BOOKSHELF4, COFFIN);
		addAttackId(TRIOL_PAWN);
		TRIOL_PAWN_LOC.put(BROKEN_BOOKSHELF2, new Location(47142, -35941, -1623));
		TRIOL_PAWN_LOC.put(BROKEN_BOOKSHELF3, new Location(50055, -47020, -3396));
		TRIOL_PAWN_LOC.put(BROKEN_BOOKSHELF4, new Location(59712, -47568, -2720));
	}
	
	@Override
	public String onTalk(L2Npc npc, L2PcInstance talker)
	{
		final QuestState qs = talker.getQuestState(qn);
		String htmltext = getNoQuestMsg();
		
		switch (qs.getState())
		{
			case STATE_CREATED:
			{
				if (npc.getNpcId() == PRIEST_BENEDICT)
				{
					QuestState q24 = talker.getQuestState(Q024_InhabitantsOfTheForestOfTheDead.class.getSimpleName());
					if ((q24 != null) && q24.isCompleted() && (talker.getLevel() >= MIN_LVL))
						htmltext = "31349-01.htm";
					else
						htmltext = "31349-02.html";
				}
				break;
			}
			case STATE_STARTED:
			{
				switch (npc.getNpcId())
				{
					case PRIEST_BENEDICT:
					{
						switch (qs.getMemoState())
						{
							case 1:
							{
								htmltext = "31349-03a.html";
								break;
							}
							case 2:
							{
								htmltext = "31349-11.html";
								break;
							}
						}
						break;
					}
					case MYSTERIOUS_WIZARD:
					{
						switch (qs.getMemoState())
						{
							case 1:
							{
								if (!qs.hasQuestItems(TOTEM_DOLL2))
								{
									qs.giveItems(TOTEM_DOLL2, 1);
									qs.set("cond", "3");
									htmltext = "31522-01.html";
								}
								else
									htmltext = "31522-02.html";
								break;
							}
							case 6:
							{
								if (qs.hasQuestItems(GEMSTONE_KEY))
									htmltext = "31522-03.html";
								break;
							}
							case 9:
							{
								if (qs.hasQuestItems(CONTRACT))
								{
									qs.set("cond", "10");
									htmltext = "31522-06.html";
								}
								break;
							}
							case 16:
							{
								htmltext = "31522-06a.html";
								break;
							}
							case 19:
							{
								htmltext = "31522-12.html";
								break;
							}
							case 20:
							{
								htmltext = "31522-14.html";
								break;
							}
							case 24:
							{
								htmltext = "31522-15.html";
								break;
							}
							case 23:
							{
								htmltext = "31522-15a.html";
								break;
							}
							default:
							{
								if ((qs.getMemoState() % 100) == 7)
									htmltext = "31522-05.html";
								break;
							}
						}
						break;
					}
					case HIGH_PRIEST_AGRIPEL:
					{
						switch (qs.getMemoState())
						{
							case 2:
							{
								htmltext = "31348-01.html";
								break;
							}
							case 3:
							{
								htmltext = "31348-03.html";
								break;
							}
							case 6:
							{
								htmltext = "31348-08a.html";
								break;
							}
							case 20:
							{
								if (qs.hasQuestItems(TOTEM_DOLL3))
									htmltext = "31348-09.html";
								break;
							}
							case 21:
							{
								htmltext = "31348-10a.html";
								break;
							}
							case 22:
							{
								htmltext = "31348-15.html";
								break;
							}
							case 23:
							{
								htmltext = "31348-18.html";
								break;
							}
							case 24:
							{
								htmltext = "31348-19.html";
								break;
							}
						}
						break;
					}
					case BROKEN_BOOKSHELF2:
					case BROKEN_BOOKSHELF3:
					case BROKEN_BOOKSHELF4:
					{
						if ((qs.getMemoState() % 100) == 7)
							htmltext = "31533-01.html";
						else if ((qs.getMemoState() % 100) >= 9)
							htmltext = "31533-02.html";
						else if (qs.isMemoState(8))
							htmltext = "31533-04.html";
						break;
					}
					case MAID_OF_LIDIA:
					{
						switch (qs.getMemoState())
						{
							case 9:
							{
								if (qs.hasQuestItems(CONTRACT))
									htmltext = "31532-01.html";
								break;
							}
							case 10:
							{
								htmltext = "31532-03.html";
								break;
							}
							case 11:
							{
								qs.playSound("SkillSound5.horror_01");
								htmltext = "31532-08.html";
								break;
							}
							case 12:
							{
								if (qs.hasQuestItems(LIDAS_DRESS))
								{
									qs.takeItems(LIDAS_DRESS, -1);
									qs.setMemoState(13);
									qs.set("cond", "14");
									htmltext = "31532-09.html";
								}
								break;
							}
							case 13:
							{
								qs.setMemoStateEx(1, 0);
								qs.playSound("ChrSound.FDElf_Cry");
								htmltext = "31532-10.html";
								break;
							}
							case 14:
							{
								htmltext = "31532-13.html";
								break;
							}
							case 15:
							{
								htmltext = "31532-18.html";
								break;
							}
							case 16:
							{
								htmltext = "31532-22.html";
								break;
							}
							case 23:
							{
								htmltext = "31532-23.html";
								break;
							}
							case 24:
							{
								htmltext = "31532-24.html";
								break;
							}
						}
						break;
					}
					case TOMBSTONE:
					{
						switch (qs.getMemoState())
						{
							case 11:
							{
								htmltext = "31531-01.html";
								break;
							}
							case 12:
							{
								htmltext = "31531-03.html";
								break;
							}
						}
						break;
					}
					case COFFIN:
					{
						if (qs.isMemoState(11))
						{
							qs.giveItems(LIDAS_DRESS, 1);
							cancelQuestTimer("DESPAWN_BOX", npc, talker);
							startQuestTimer("DESPAWN_BOX", 3000, npc, talker,false);
							qs.setMemoState(12);
							qs.set("cond", "13");
							htmltext = "31536-01.html";
						}
						break;
					}
				}
				break;
			}
			case STATE_COMPLETED:
			{
				if (npc.getNpcId() == PRIEST_BENEDICT)
					htmltext = super.getAlreadyCompletedMsg();
				break;
			}
		}
		return htmltext;
	}
	
	@Override
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{		
		final QuestState qs = player.getQuestState(qn);
		String htmltext = getNoQuestMsg();
		
		switch (event)
		{
			case "31349-06.html":
			case "31349-07.html":
			case "31349-08.html":
			case "31349-09.html":
			case "31522-08.html":
			case "31522-09.html":
			case "31522-07.html":
			case "31522-11.html":
			case "31348-04.html":
			case "31348-05.html":
			case "31348-06.html":
			case "31348-11.html":
			case "31348-07.html":
			case "31348-12.html":
			case "31348-14.html":
			case "31532-04.html":
			case "31532-05.html":
			case "31532-06.html":
			case "31532-14.html":
			case "31532-15.html":
			case "31532-16.html":
			case "31532-19.html":
			case "31532-20.html":
			{
				htmltext = event;
				break;
			}
			case "31349-03.html":
			{
				if (qs.isCreated() && player.hasQuestCompleted(Q024_InhabitantsOfTheForestOfTheDead.class.getSimpleName()) && (player.getLevel() >= MIN_LVL))
				{
					qs.setMemoState(1);
					qs.setState(STATE_STARTED);
					qs.set("cond", "1");
					qs.playSound(QuestState.SOUND_ACCEPT);				
					htmltext = event;
				}
				break;
			}
			case "31349-05.html":
			{
				if (qs.isMemoState(1))
				{
					if (qs.hasQuestItems(TOTEM_DOLL2))
						htmltext = "31349-04.html";
					else
					{
						qs.set("cond", "2");
						htmltext = event;
					}
				}
				break;
			}
			case "31349-10.html":
			{
				if (qs.isMemoState(1) && qs.hasQuestItems(TOTEM_DOLL2))
				{
					qs.setMemoState(2);
					qs.set("cond", "4");
					htmltext = event;
				}
				break;
			}
			case "31522-04.html":
			{
				if (qs.isMemoState(6) && qs.hasQuestItems( GEMSTONE_KEY))
				{
					qs.setMemoState(7);
					qs.setMemoStateEx(1, 20);
					qs.set("cond", "6");
					htmltext = event;
				}
				break;
			}
			case "31522-10.html":
			{
				if (qs.isMemoState(16))
				{
					qs.setMemoState(19);
					htmltext = event;
				}
				break;
			}
			case "31522-13.html":
			{
				if (qs.isMemoState(19))
				{
					qs.setMemoState(20);
					qs.set("cond", "16");
					htmltext = event;
				}
				break;
			}
			case "31522-16.html":
			{
				if (qs.isMemoState(24))
				{
					qs.takeItems(MAP_FOREST_OF_THE_DEAD, -1);
					qs.rewardItems(EARING_OF_BLESSING, 1);
					qs.rewardItems(NECKLACE_OF_BLESSING, 1);
					qs.rewardExpAndSp(572277, 53750);
					qs.exitQuest(false);
					htmltext = event;
				}
				break;
			}
			case "31348-02.html":
			{
				if (qs.isMemoState(2))
				{
					qs.takeItems(TOTEM_DOLL2, -1);
					qs.setMemoState(3);
					htmltext = event;
				}
				break;
			}
			case "31348-08.html":
			{
				if (qs.isMemoState(3))
				{
					qs.giveItems(GEMSTONE_KEY, 1);
					qs.setMemoState(6);
					qs.set("cond", "5");
					htmltext = event;
				}
				break;
			}
			case "31348-10.html":
			{
				if (qs.isMemoState(20) && qs.hasQuestItems(TOTEM_DOLL3))
				{
					qs.takeItems(TOTEM_DOLL3, -1);
					qs.setMemoState(21);
					htmltext = event;
				}
				break;
			}
			case "31348-13.html":
			{
				if (qs.isMemoState(21))
				{
					qs.setMemoState(22);
					htmltext = event;
				}
				break;
			}
			case "31348-16.html":
			{
				if (qs.isMemoState(22))
				{
					qs.setMemoState(23);
					qs.set("cond", "17");
					htmltext = event;
				}
				break;
			}
			case "31348-17.html":
			{
				if (qs.isMemoState(22))
				{
					qs.setMemoState(24);
					qs.set("cond", "18");
					htmltext = event;
				}
				break;
			}
			case "31533-04.html":
			{
				if (qs.getMemoStateEx(npc.getNpcId()) != 0)
					htmltext = "31533-03.html";
				else if (Rnd.get(60) > qs.getMemoStateEx(1))
				{
					qs.setMemoStateEx(1, qs.getMemoStateEx(1) + 20);
					qs.setMemoStateEx(npc.getNpcId(), 1);
					htmltext = "31533-03.html";
				}
				else
				{
					qs.setMemoState(8);
					htmltext = event;
					qs.playSound(QuestState.AMDSOUND_HORROR_02);
				}
				break;
			}
			case "31533-05.html":
			{
				if (qs.isMemoState(8))
				{
					if (!qs.hasQuestItems(TOTEM_DOLL3))
					{
						final int step = qs.getInt("step");
						if (step == 0)
						{
							final L2Npc triyol = addSpawn(TRIOL_PAWN, TRIOL_PAWN_LOC.get(npc.getNpcId()), true, 0,false);
							triyol.setScriptValue(player.getObjectId());
							startQuestTimer("SAY_TRIYOL", 500, triyol, player,false);
							startQuestTimer("DESPAWN_TRIYOL", 120000, triyol, player,false);
							triyol.getAI().setIntention(CtrlIntention.AI_INTENTION_ATTACK, player);							
							htmltext = event;
							qs.set("cond", "7");
							qs.set("step", "1");
						}
						else if (step == 2)
							htmltext = "31533-07.html";
						else
							htmltext = "31533-06.html";
					}
					else
						htmltext = "31533-08.html";
				}
				break;
			}
			case "31533-09.html":
			{
				if (qs.isMemoState(8) && qs.hasQuestItems(TOTEM_DOLL3, GEMSTONE_KEY))
				{
					qs.giveItems(CONTRACT, 1);
					qs.takeItems(GEMSTONE_KEY, -1);
					qs.setMemoState(9);
					qs.set("cond", "9");
					htmltext = event;
				}
				break;
			}
			case "SAY_TRIYOL":
			{
				npc.broadcastPacket(new NpcSay(npc.getObjectId(), ChatType.GENERAL.getClientId(), npc.getNpcId(),"That box was sealed by my master."+player.getName()+" Don't touch it!"));
				break;
			}
			case "DESPAWN_TRIYOL":
			{
				npc.deleteMe();
				break;
			}
			case "31532-02.html":
			{
				if (qs.isMemoState(9) && qs.hasQuestItems(CONTRACT))
				{
					qs.takeItems(CONTRACT, -1);
					qs.setMemoState(10);
					htmltext = event;
				}
				break;
			}
			case "31532-07.html":
			{
				if (qs.isMemoState(10))
				{
					qs.setMemoState(11);
					qs.playSound("SkillSound5.horror_01");
					qs.set("cond", "11");
					htmltext = event;
				}
				break;
			}
			case "31532-11.html":
			{
				if (qs.isMemoState(13))
				{
					final int memoStateEx = qs.getMemoStateEx(1);
					if (memoStateEx <= 3)
					{
						qs.setMemoStateEx(1, memoStateEx + 1);
						qs.playSound("ChrSound.FDElf_Cry");
						htmltext = event;
					}
					else
					{
						qs.setMemoState(14);
						htmltext = "31532-12.html";
					}
				}
				break;
			}
			case "31532-17.html":
			{
				if (qs.isMemoState(14))
				{
					qs.setMemoState(15);
					htmltext = event;
				}
				break;
			}
			case "31532-21.html":
			{
				if (qs.isMemoState(15))
				{
					qs.setMemoState(16);
					qs.set("cond", "15");
					htmltext = event;
				}
				break;
			}
			case "31532-25.html":
			{
				if (qs.isMemoState(23))
				{
					qs.takeItems(MAP_FOREST_OF_THE_DEAD, -1);
					qs.rewardItems(EARING_OF_BLESSING, 1);
					qs.rewardItems(RING_OF_BLESSING, 2);
					qs.rewardExpAndSp(572277, 53750);
					qs.exitQuest(false);
					htmltext = event;
				}
				break;
			}
			case "31531-02.html":
			{
				if (qs.isMemoState(11))
				{
					final L2Npc box = addSpawn(COFFIN, COFFIN_LOC, true, 0,false);
					qs.getQuest().startQuestTimer("DESPAWN_BOX", 20000, box, player,false);
					qs.set("cond", "12");
					htmltext = event;
				}
				break;
			}
			case "DESPAWN_BOX":
			{
				npc.deleteMe();
				break;
			}
		}
		return htmltext;
	}

	@Override
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon)
	{
		if (npc.getCurrentHp() <= (0.30 * npc.getMaxHp()))
		{
			final QuestState qs = attacker.getQuestState(qn);

			if (qs.isMemoState(8) && !qs.hasQuestItems(TOTEM_DOLL3) && (attacker.getObjectId() == npc.getScriptValue()))
			{
				qs.giveItems(TOTEM_DOLL3, 1);
				qs.set("cond", "8");
				qs.set("step", "2");
				npc.broadcastPacket(new NpcSay(npc.getObjectId(), ChatType.GENERAL.getClientId(), npc.getNpcId(), "You've ended my immortal life! You've protected by the feudal lord, aren't you?"));
				npc.deleteMe();
			}
		}
		return super.onAttack(npc, attacker, damage, isSummon);
	}
	
	public static void main(String[] args)
	{
		new Q025_HidingBehindTheTruth();
	}
}
