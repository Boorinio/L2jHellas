...
public class ScrollOfEscape implements IItemHandler
{
	...
	{
	736, 1830, 1829, 1538, 3958, 5858, 5859, 7117, 7118, 7119, 7120, 7121, 7122, 7123, 7124, 7125, 7126, 7127, 7128, 7129, 7130, 7131, 7132, 7133, 7134, 7135, 7554, 7555, 7556, 7557, 7558, 7559, 7618, 7619
	};
		...
		public void run()
		{
			...
			if (Config.MOD_GVE_ENABLE_FACTION)
			{
				if (_activeChar.isgood())
					_activeChar.teleToLocation(Config.GOODX, Config.GOODY, Config.GOODZ, true); // good
																								// Base
					
				if (_activeChar.isevil())
					_activeChar.teleToLocation(Config.EVILX, Config.EVILY, Config.EVILZ, true); // evil
																								// Base
			}
