...
public abstract class L2Skill
{
    ...
    public final boolean getWeaponDependancy(L2Character activeChar)
    {
        ...
        if (activeChar.getSecondaryWeaponItem() != null) mask |= activeChar.getSecondaryWeaponItem().getItemType().mask();
     ...
     * ONE: The skill can only be used on the L2PcInstance targeted, or on the caster if it's a L2PcInstance and no L2PcInstance targeted</li>
     * SELF
     * HOLY, UNDEAD
     * PET
     * AURA, AURA_CLOSE
        ...
        switch (targetType)
        {
                    ...
                    if (clan != null)
                    {
                        ...
                        for (L2Object newTarget : activeChar.getKnownList().getKnownObjects().values())
                        {
                          ...
                          //check if allow interference is allowed if player is not on event but target is on event
    						...
    						{
    							if ((newTarget._inEventTvT && !player._inEventTvT) || (!newTarget._inEventTvT && player._inEventTvT)) continue;
            ...
            case TARGET_AREA_CORPSE_MOB:
            {
                ...
                if (activeChar.getKnownList() != null)
                	for (L2Object obj : activeChar.getKnownList().getKnownObjects().values())
                    {
                        ...
                        if (!(obj instanceof L2Attackable || obj instanceof L2PlayableInstance) || ((L2Character) obj).isDead() || ((L2Character) obj) == activeChar) continue;
            ...
            case TARGET_AREA_UNDEAD:
            {
                ...
                if (cha != null && cha.getKnownList() != null)
                    ...
                    {
                        if (obj == null) continue;
                    	if (obj instanceof L2NpcInstance) target = (L2NpcInstance) obj;
                    	else if (obj instanceof L2SummonInstance) target = (L2SummonInstance) obj;
                    	else continue;
            ...
            case TARGET_AREA_ANGEL:
            {
                ...
                if (cha != null && cha.getKnownList() != null)
                    for (L2Object obj : cha.getKnownList().getKnownObjects().values())
                    {
                        ...
                        if (!target.isAlikeDead()) // If target is not dead/fake death and not self
                        {
                            if (target.isUndead()) continue; // Go to next obj if obj isn't a angel
                            if (!Util.checkIfInRange(radius, cha, obj, true)) // Go to next obj if obj isn't in range
                                continue;
