...
public final class QuestState
{
	 ...
	 * appropriate packets to the client<BR>
	 * <BR>
	 * <U><I>Actions :</I></U><BR>
	 * <LI>Check if the new progress number resets the quest to a previous (smaller) step</LI> <LI>If not, check if quest progress steps have been skipped</LI> <LI>If skipped,
	 ...
	 * @param old
	 *        : int indicating the previously noted step
	 *        For more info on the variable communicating the progress steps to the client, please see
	 * @link com.l2jhellas.loginserver.serverpacket.QuestList
	 */
	...
	{
		if (getPlayer().getPremiumService() == 1)
			dropChance *= Config.RATE_DROP_QUEST / ((getPlayer().getParty() != null) ? getPlayer().getParty().getMemberCount() : 1);
		else
			dropChance *= Config.PREMIUM_RATE_DROP_QUEST / ((getPlayer().getParty() != null) ? getPlayer().getParty().getMemberCount() : 1);
