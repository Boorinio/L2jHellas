			...
			PreparedStatement ps1 = con.prepareStatement("SELECT COUNT(*) FROM accounts WHERE login=?;"))
		{
			...
			try (ResultSet rs = ps1.executeQuery())
			{
				...
				else if (rs.getInt(1) > 0)
				{
					// Exist
					// Update
					try (PreparedStatement ps2 = con.prepareStatement("UPDATE accounts SET access_level=? WHERE login=?;"))
					{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			else if (rset.getInt(1) > 0)
			{
				...
				for (int index = 0; index < objIds.size(); index++)
				{
					...
					// recommends
					statement.close();
					statement = con.prepareStatement("DELETE FROM character_reco_bonus WHERE char_id=?;");
					statement.setString(1, objIds.get(index));
					statement.executeUpdate();
				...
				// characters
				statement.close();
				statement = con.prepareStatement("DELETE FROM account_data WHERE account_name=?;");
				statement.setString(1, account);
				statement.executeUpdate();
