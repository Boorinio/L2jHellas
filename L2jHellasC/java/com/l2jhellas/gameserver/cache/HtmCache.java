...
public class HtmCache
{
	...
	public void reload(File f)
	{
		...
		{
			_log.log(Level.INFO, getClass().getSimpleName() + ": Html cache start...");
			parseDir(f);
			_log.log(Level.INFO, getClass().getSimpleName() + ": " + String.format("%.3f", getMemoryUsage()) + " megabytes on " + getLoadedFiles() + " files loaded");
		}
		else
		{
			_cache.clear();
			_loadedFiles = 0;
			_bytesBuffLen = 0;
			_log.log(Level.INFO, getClass().getSimpleName() + ": Running lazy cache");
		}
	}
	...
	{
		parseDir(f);
		_log.log(Level.INFO, getClass().getSimpleName() + ": Reloaded specified path.");
	}
