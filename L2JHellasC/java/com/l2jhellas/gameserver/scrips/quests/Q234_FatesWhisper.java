...
public class Q234_FatesWhisper extends Quest
{
	...
	// Chest Spawn
	private static final Map<Integer, Integer> CHEST_SPAWN = new HashMap<Integer, Integer>();
	{
	...
	// Weapons
	private static final Map<Integer, String> WEAPONS = new HashMap<Integer, String>();
	{
		...
		else if (event.startsWith("selectAGrade_"))
		{
			if (st.getInt("bypass") == 1)
			{
				...
				if (st.hasQuestItems(itemId))
				{
					...
					htmltext = getHtmlText("31002-12.htm").replace("%weaponname%", ItemTable.getInstance().getTemplate(aGradeItemId).getItemName());
