...
public class Frintezza extends AbstractNpcAI
{
	...
	private final int SCARLET1 = 29046;
	private final int SCARLET2 = 29047;
	private final int FRINTEZZA = 29045;
	private final int GUIDE = 32011;
	private final int CUBE = 29061;

	//Frintezza Status Tracking :
	private final byte DORMANT = 0;		//Frintezza is spawned and no one has entered yet. Entry is unlocked
	private final byte WAITING = 1;		//Frintezza is spawend and someone has entered, triggering a 30 minute window for additional people to enter
												//before he unleashes his attack. Entry is unlocked
	private final byte FIGHTING = 2; 	//Frintezza is engaged in battle, annihilating his foes. Entry is locked
	private final byte DEAD = 3;			//Frintezza has been killed. Entry is locked

	private long _LastAction = 0;
	private int _Angle = 0;
	private int _Heading = 0;
	private int _LocCycle = 0;
	private int _Bomber = 0;
	private int _CheckDie = 0;
	private int _OnCheck = 0;
	private int _OnSong = 0;
	private int _Abnormal = 0;
	private int _OnMorph = 0;
	private int _Scarlet_x = 0;
	private int _Scarlet_y = 0;
	private int _Scarlet_z = 0;
	private int _Scarlet_h = 0;
	private int _SecondMorph = 0;
	private int _ThirdMorph = 0;
	private int _KillHallAlarmDevice = 0;
	...
	private List<L2Npc> _Room1Mobs = new ArrayList<L2Npc>();
	private List<L2Npc> _Room2Mobs = new ArrayList<L2Npc>();
	private List<L2Attackable> Minions = new ArrayList<L2Attackable>();
	...
	public Frintezza()
	{
		...
		int status = GrandBossManager.getBossStatus(FRINTEZZA);
		if (status == DEAD)
		{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			if (GrandBossManager.getBossStatus(FRINTEZZA) == FIGHTING)
			{
				if (!_Zone.isInsideZone(npc))
					npc.teleToLocation(174232,-88020,-5116,false);
		...
		else if (event.equalsIgnoreCase("camera_3"))
		{
			...
			GrandBossManager.addBoss(frintezza);
		...
		{
			weakScarlet.teleToLocation(174232, -88020, -5110,false);
		}
	...
	public String onTalk (L2Npc npc, L2PcInstance player)
	{
		...
		String htmltext = "";
		if (GrandBossManager.getBossStatus(FRINTEZZA) == DEAD)
		{
		...
		else if (GrandBossManager.getBossStatus(FRINTEZZA) == DORMANT)
		{
			...
			else
			{
				...
				GrandBossManager.setBossStatus(FRINTEZZA, WAITING);
	...
	public String onAttack (L2Npc npc, L2PcInstance attacker, int damage, boolean isPet)
	{
		...
		if (npc.getNpcId() == SCARLET1 && _SecondMorph == 0 && _ThirdMorph == 0 && _OnMorph == 0 && npc.getCurrentHp() < npc.getMaxHp() * 0.75 && GrandBossManager.getBossStatus(FRINTEZZA) == FIGHTING)
		{
			startQuestTimer("attack_stop", 0, frintezza, null,false);
		...
		else if (npc.getNpcId() == SCARLET1 && _SecondMorph == 1 && _ThirdMorph == 0 && _OnMorph == 0 && npc.getCurrentHp() < npc.getMaxHp() * 0.5 && GrandBossManager.getBossStatus(FRINTEZZA) == FIGHTING)
		{
			startQuestTimer("attack_stop", 0, frintezza, null,false);
		...
		else if (npc.getNpcId() == SCARLET2 && _SecondMorph == 1 && _ThirdMorph == 1 && _OnCheck == 0 && damage >= npc.getCurrentHp() && GrandBossManager.getBossStatus(FRINTEZZA) == FIGHTING)
		{
			_OnCheck = 1;
	...
	public String onKill (L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		...
		else if (npc.getNpcId() == SCARLET2 && _OnCheck == 0 && GrandBossManager.getBossStatus(FRINTEZZA) == FIGHTING)
		{
			_OnCheck = 1;
			startQuestTimer("stop_pc", 0, npc, null,false);
			startQuestTimer("stop_npc", 0, npc, null,false);
		...
		else if (npc.getNpcId() == SCARLET2 && _OnCheck == 1 && GrandBossManager.getBossStatus(FRINTEZZA) == FIGHTING)
		{
			cancelQuestTimers("loc_check");
			cancelQuestTimers("spawn_minion");
			cancelQuestTimers("frintezza_despawn");
			startQuestTimer("clean", 30000, npc, null,false);
			startQuestTimer("close", 30000, npc, null,false);
			startQuestTimer("room3_del", 60000, npc, null,false);
			startQuestTimer("minions_despawn", 60000, npc, null,false);
			startQuestTimer("remove_players", 900000, npc, null,false);
		...
		else if (npc.getNpcId() == 18339)
		{
			...
			if (_KillDarkChoirPlayer == 2)
			{
				...
				if (outside == 0)
				{
					...
					startQuestTimer("waiting", 180000, npc, null,false);
					cancelQuestTimers("room_final");
				}
				else
				{
