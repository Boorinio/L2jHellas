...
public class AdminSearch implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		if (command.equals("admin_search"))
			html.replace("%list%", "<center><br><br><br>first select a key word</center>");
	...
	public StringBuilder getList(L2PcInstance activeChar, int page, String search)
	{
		List<L2Item> items = Arrays.asList(ItemTable.getInstance().getTemplates()).stream().filter(item -> item != null && matches(item.getItemName(), search)).collect(Collectors.toList());
		...
		sb.append("<td align=right width=70>" + (page > 1 ? "<button value=\"< PREV\" action=\"bypass admin_search " + (page - 1) + " " + search + "\" width=65 height=19 back=sek.cbui94 fore=sek.cbui92>" : "") + "</td>");
		sb.append("<td align=center width=100>Page " + page + "</td>");
		sb.append("<td align=left width=70>" + (page < max ? "<button value=\"NEXT >\" action=\"bypass admin_search " + (page + 1) + " " + search + "\" width=65 height=19 back=sek.cbui94 fore=sek.cbui92>" : "") + "</td>");
		sb.append("</tr></table><img src=L2UI.SquareGray width=280 height=1>");
		return sb;
	}
