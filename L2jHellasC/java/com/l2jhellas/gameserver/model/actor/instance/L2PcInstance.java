...
{	       
	...
	public class AIAccessor extends L2Character.AIAccessor {
		...
		public void doCast(L2Skill skill) {
			...
			default: {
				L2Object mainTarget = skill
						.getFirstOfTargetList(L2PcInstance.this);
	...
	/**
	 * Starts battle force / spell force on target.<br>
	 * <br>
	 * 
	 * @param caster
	 * @param force
	...
	private PunishLevel _punishLevel = PunishLevel.NONE; // TODO Clean up,
															// delete old
															// methods and add
															// this one...
	...
	public enum PunishLevel {
		NONE(0, ""), CHAT(1, "chat banned"), JAIL(2, "jailed"), CHAR(3,
				"banned"), ACC(4, "banned");
	 ...
	 * <li>Add the L2PcInstance object in _allObjects</li> <li>Set the x,y,z
	 * position of the L2PcInstance and make it invisible</li> <li>Update the
	 ...
	 * copy basic Calculator set to this L2PcInstance</li> <li>Set the name of
	...
	public void setTemplate(ClassId newclass) {
		super
				.setTemplate(CharTemplateTable.getInstance().getTemplate(
						newclass));
	}
	...
	public void unregisterRecipeList(int recipeId) {
		...
		for (L2ShortCut sc : allShortCuts) {
			if (sc != null && sc.getId() == recipeId
					&& sc.getType() == L2ShortCut.TYPE_RECIPE)
		...
		for (Quest quest : npc.getTemplate().getEventQuests(Quest.QuestEventType.ON_ATTACK)) 
		{
			...
			if (getQuestState(quest.getName()) != null) {
				...
				if (states == null)
					...
					states = addToQuestStateArray(states, getQuestState(quest
							.getName()));
			}
		}
	...
	public QuestState[] getQuestsForTalk(int npcId) {
		...
		Quest[] quests = NpcTable.getInstance().getTemplate(npcId)
				.getEventQuests(Quest.QuestEventType.ON_TALK);
		if (quests != null) {
	...
	public void updatePvPFlag(int value) {
		...
		for (L2PcInstance target : getKnownList().getKnownPlayers().values()) {
			...
			if (getPet() != null)
				target.sendPacket(new RelationChanged(getPet(),
						getRelation(this), isAutoAttackable(target)));
		}
	}
	...
	public int getMaxLoad() {
		...
		return (int) calcStat(Stats.MAX_LOAD, baseLoad
				* Config.ALT_WEIGHT_LIMIT, this, null);
	}
	...
	public void refreshOverloaded() 
	{
		...
		if (maxLoad > 0) 
		{
			...
			if (weightproc < 500 || _dietMode || Config.DISABLE_WEIGHT_PENALTY) newWeightPenalty = 0;
	 ...
	 public void CheckIfWeaponIsAllowed()
	 {
	  ...
	  // Check to rest to assure current effect meets weapon requirements.
		  ...
		  { 
			...
			if (Config.DEBUG) _log.info("   | Skill " + effectSkill.getName() + " has been disabled for (" + getName() + "); Reason: Incompatible Weapon Type."); 
			currenteffect.exit(); 
			  	 	         } 
			  	 	    }		  	 	 
			continue; 
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the Level of the L2PcInstance</li> <li>If L2PcInstance Level is
	 * 5, remove beginner Lucky skill</li> <li>Add the Expertise skill
	...
	public Race getRace() {
		...
		L2PcTemplate charTemp = CharTemplateTable.getInstance().getTemplate(
				_baseClass);
		...
		if (count > 0) {
			...
			if (sendMessage) {
				SystemMessage sm = new SystemMessage(
						SystemMessageId.DISSAPEARED_ADENA);
				sm.addNumber(count);
				sendPacket(sm);
			}
		}
	...
	public void addItem(String process, L2ItemInstance item,
		...
		if (item.getCount() > 0) {
			...
			if (sendMessage) {
				if (item.getCount() > 1) {
					SystemMessage sm = new SystemMessage(
							SystemMessageId.YOU_PICKED_UP_S1_S2);
					sm.addItemName(item.getItemId());
					sm.addNumber(item.getCount());
					sendPacket(sm);
				} else if (item.getEnchantLevel() > 0) {
					SystemMessage sm = new SystemMessage(
							SystemMessageId.YOU_PICKED_UP_A_S1_S2);
					sm.addNumber(item.getEnchantLevel());
					sm.addItemName(item.getItemId());
					sendPacket(sm);
				} else {
					SystemMessage sm = new SystemMessage(
							SystemMessageId.YOU_PICKED_UP_S1);
					sm.addItemName(item.getItemId());
					sendPacket(sm);
				}
			}
		...
		if (count > 0) {
			...
			if (sendMessage
					&& (!isCastingNow()
							&& ItemTable.getInstance().createDummyItem(itemId)
									.getItemType() == L2EtcItemType.HERB || ItemTable
							.getInstance().createDummyItem(itemId)
							.getItemType() != L2EtcItemType.HERB)) {
				if (count > 1) {
					if (process.equalsIgnoreCase("sweep")
							|| process.equalsIgnoreCase("Quest")) {
						SystemMessage sm = new SystemMessage(
								SystemMessageId.EARNED_S2_S1_S);
																										...
																										// :]
			{
				if (!isCastingNow()) {
					...
					if (handler == null)
						_log
								.warning("No item handler registered for Herb - item ID "
										+ herb.getItemId() + ".");
					else {
					...
					ThreadPoolManager.getInstance().scheduleAi(
							new HerbTask(process, itemId, count, reference,
									sendMessage), _herbstask);
				}
			} else {
				// Add the item to inventory
				L2ItemInstance item = _inventory.addItem(process, itemId,
	...
	public boolean destroyItem(String process, int objectId, int count,
		...
		if (item == null
				|| item.getCount() < count
				|| _inventory.destroyItem(process, objectId, count, this,
						reference) == null) {
			if (sendMessage)
				sendPacket(new SystemMessage(SystemMessageId.NOT_ENOUGH_ITEMS));
	...
	public L2ItemInstance transferItem(String process, int objectId, int count,
		...
		if (target instanceof PcInventory) {
			...
			playerSU.addAttribute(StatusUpdate.CUR_LOAD, targetPlayer
					.getCurrentLoad());
	...
	public boolean dropItem(String process, L2ItemInstance item,
		...
		item.dropMe(this, getClientX() + Rnd.get(50) - 25, getClientY()
				+ Rnd.get(50) - 25, getClientZ() + 20);
		...
		{
			if ((item.isEquipable() && Config.DESTROY_EQUIPABLE_PLAYER_ITEM) || !item.isEquipable()) ItemsAutoDestroy.getInstance().addItem(item);
		}
		if (Config.DESTROY_DROPPED_PLAYER_ITEM) 
		{
			if (!item.isEquipable() || (item.isEquipable() && Config.DESTROY_EQUIPABLE_PLAYER_ITEM)) item.setProtected(false);
			else item.setProtected(true);
		} 
		else item.setProtected(true);
	...
	public L2ItemInstance checkItemManipulation(int objectId, int count,
		...
		// Exploit)
		if (item.isAugmented() && (isCastingNow() || this.isCastingNow()))
			return null;
	 ...
	 * <BR>
	 * <li>Set the target of the player</li> <li>Send a Server->Client packet
	 * MyTargetSelected to the player (display the select window)</li><BR>
	 * <BR>
	 * 
	 ...
	 * the select window)</li> <li>If this L2PcInstance has a Private Store,
	 * notify the player AI with AI_INTENTION_INTERACT</li> <li>If this
	 * L2PcInstance is autoAttackable, notify the player AI with
			...
			if (getPrivateStoreType() != 0) {
				...
				if (isAutoAttackable(player) || (player._inEventTvT && TvT._started) || (player._inEventCTF && CTF._started) || (player._inEventDM && DM._started) || (player._inEventVIP && VIP._started) && !isGM())
				{
					if (Config.ALLOW_CHAR_KILL_PROTECT) 
					{
						...
						if ((siege != null && siege.getIsInProgress())) {
							if (player.getLevel() > 20
									&& ((L2Character) player.getTarget())
											.getLevel() < 20) {
								player
										.sendMessage("You can only hit players within your grade.");
								player.sendPacket(new ActionFailed());
							}
					...
					if (Config.ALT_PLAYER_PROTECTION) {
						if (player.getLevel() > Config.ALT_PLAYER_PROTECTION_LEVEL
								&& ((L2Character) player.getTarget())
										.getLevel() > Config.ALT_PLAYER_PROTECTION_LEVEL
								&& !player.isInOlympiadMode()) {
							player
									.sendMessage("You Can't Hit a Player That Is Lower Level From You. Target's Level: "
											+ String
													.valueOf(Config.ALT_PLAYER_PROTECTION_LEVEL));
							player.sendPacket(new ActionFailed());
						}
					}
	 ...
	 * and only current HP, MP and Level to all other L2PcInstance of the Party.<BR>
	 * <BR>
	 * 
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 ...
	 * and Private Data)</li> <li>Send a Server->Client packet CharInfo to all
	 * L2PcInstance in _KnownPlayers of the L2PcInstance (Public data only)</li><BR>
	 * <BR>
	 * 
	 ...
	 * a L2PcInstance)</li> <li>If attack isn't aborted, send a message system
	 * (critical hit, missed...) to attacker/target if they are L2PcInstance</li>
	 * <li>If attack isn't aborted and hit isn't missed, reduce HP of the target
	 * and calculate reflection damage to reduce HP of attacker if necessary</li>
	 ...
	 * Remove the L2ItemInstance from the world and send server->client GetItem
	 * packets</li> <li>Send a System Message to the L2PcInstance :
	 * YOU_PICKED_UP_S1_ADENA or YOU_PICKED_UP_S1_S2</li> <li>Add the Item to
	 * the L2PcInstance inventory</li> <li>Send a Server->Client packet
	 * InventoryUpdate to this L2PcInstance with NewItem (use a new slot) or
	 * ModifiedItem (increase amount)</li> <li>Send a Server->Client packet
	...
	protected void doPickupItem(L2Object object) {
		...
		synchronized (target) {
			...
			if (isInvul() && !isGM()) {
				...
				SystemMessage smsg = new SystemMessage(
						SystemMessageId.FAILED_TO_PICKUP_S1);
			...
			if (target.getOwnerId() != 0
				...
				if (target.getItemId() == 57) {
					SystemMessage smsg = new SystemMessage(
							SystemMessageId.FAILED_TO_PICKUP_S1_ADENA);
		...
		if (target.getItemType() == L2EtcItemType.HERB) {
			IItemHandler handler = ItemHandler.getInstance().getItemHandler(
					target.getItemId());
			if (handler == null)
		...
		// Cursed Weapons are not distributed
			...
			if (target.getItemType() instanceof L2ArmorType
					|| target.getItemType() instanceof L2WeaponType) {
				if (target.getEnchantLevel() > 0) {
					SystemMessage msg = new SystemMessage(
							SystemMessageId.ATTENTION_S1_PICKED_UP_S2_S3);
					msg.addString(getName());
					...
					SystemMessage msg = new SystemMessage(
							SystemMessageId.ATTENTION_S1_PICKED_UP_S2);
					msg.addString(getName());
					msg.addItemName(target.getItemId());
					broadcastPacket(msg, 1400);
				}
	 ...
	 * it was a L2Character</li> <li>Add the L2PcInstance to the _statusListener
	 * of the new target if it's a L2Character</li> <li>Target the new L2Object
	 * (add the target to the L2PcInstance _target, _knownObject and
	...
	public void setTarget(L2Object newTarget) {
		...
		if (!isGM()) {
			...
			// Can't target and attack rift invaders if not in the same room
				...
				byte riftRoom = getParty().getDimensionalRift()
						.getCurrentRoom();

				if (newTarget != null
						&& !DimensionalRiftManager.getInstance().getRoom(
								riftType, riftRoom).checkIfInZone(
		...
		if (newTarget != null && newTarget instanceof L2Character) {
			...
			TargetSelected my = new TargetSelected(getObjectId(), newTarget
					.getObjectId(), getX(), getY(), getZ());
			broadcastPacket(my);
		}
	...
	public void EngageAnswer(int answer) {
		if (_engagerequest == false)
			...
			L2PcInstance ptarget = (L2PcInstance) L2World.getInstance()
					.findObject(_engageid);
	 ...
	 * calculated Death Penalty</li> <li>If necessary, unsummon the Pet of the
	 * killed L2PcInstance</li> <li>Manage Karma gain for attacker and Karam
	 * loss for the killed L2PcInstance</li> <li>If the killed L2PcInstance has
	 * Karma, manage Drop Item</li> <li>Kill the L2PcInstance</li><BR>
	 * <BR>
	 * 
	 * 
	 * @param i
	...
	public boolean doDie(L2Character killer) {
			...
			if (pk != null && pk._inEventTvT && _inEventTvT)
			{
				if (TvT._teleport || TvT._started)
				{
					if (!(pk._teamNameTvT.equals(_teamNameTvT)))
					{
						...
						_countTvTdies++;
						pk._countTvTkills++;
						pk.setTitle("Kills: " + ((L2PcInstance) pk)._countTvTkills);
						pk.sendPacket(ps);
				...
				if (pk == null || !pk.isCursedWeaponEquiped()) {
					...
					if (!(isInsideZone(ZONE_PVP) && !isInsideZone(ZONE_SIEGE))) {
						...
						if (isKillerPc
								...
								((L2PcInstance) killer)
										...
										.setReputationScore(
												((L2PcInstance) killer)
														.getClan()
														.getReputationScore()
														+ Config.ALT_REPUTATION_SCORE_PER_KILL,
												true);
								getClan().broadcastToOnlineMembers(
										new PledgeShowInfoUpdate(_clan));
								((L2PcInstance) killer).getClan()
							...
							if (((L2PcInstance) killer).getClan()
									.getReputationScore() > 0) // when the
																// opposing
																// sides
																// reputation
																// score is 0 or
																...
																// not decrease
							{
								_clan.setReputationScore(_clan
										.getReputationScore()
										- Config.ALT_REPUTATION_SCORE_PER_KILL,
										true);
								getClan().broadcastToOnlineMembers(
						...
						if (Config.ALT_GAME_DELEVEL) {
							...
							if (getSkillLevel(L2Skill.SKILL_LUCKY) < 0
									|| getStat().getLevel() > 9)
								deathPenalty((pk != null && getClan() != null
										&& pk.getClan() != null && pk.getClan()
										.isAtWarWith(getClanId())));
						} else {
		...
		for (L2Character character : getKnownList().getKnownCharacters())
			if (character.getForceBuff() != null
					&& character.getForceBuff().getTarget() == this)
				character.abortCast();
			...
			while (dropPercent > 0 && Rnd.get(100) < dropPercent
				...
				for (L2ItemInstance itemDrop : getInventory().getItems()) {
					...
					if (itemDrop.isAugmented()
							...
							getPet() != null
							&& getPet().getControlItemId() == itemDrop
									.getItemId() // Control Item of active pet
					)
						continue;
	...
	public void onKillUpdatePvPKarma(L2Character target) 
	{
		...
		// L2l2jhellas Faction GvE - adena reward
	...
	public void increasePkKillsAndKarma(int targLVL) 
	{
		...
		if (Config.ALLOW_PK_REWARD) {
			// Item Reward system
			addItem("Loot", Config.PK_REWARD_ITEM, Config.PK_REWARD_COUNT,
					this, true);
		...
		// Update the character's title color if they reached any of the 5 PK
		// levels.
		updatePkColor(getPkKills());
		broadcastUserInfo();
		// Send a Server->Client UserInfo packet to attacker with its Karma and
	...
	public void updatePvPStatus(L2Character target) 
	{
		...
		if ((isInDuel() && player_target.getDuelId() == getDuelId()))
			return;
		if ((!isInsideZone(ZONE_PVP) || !player_target.isInsideZone(ZONE_PVP))
				&& player_target.getKarma() == 0) {
	...
	public void restoreExp(double restorePercent) {
		if (getExpBeforeDeath() > 0) {
			...
			getStat().addExp(
					(int) Math.round((getExpBeforeDeath() - getExp())
							* restorePercent / 100));
			setExpBeforeDeath(0);
		}
	}
	 ...
	 * 
	 * Actions:
	 * Calculate the Experience loss Set the value of
	 * _expBeforeDeath Set the new Experience value of the L2PcInstance
	 ...
	 * YOU_INCREASED_YOUR_LEVEL</li> <li>Send a Server->Client packet
	 * StatusUpdate to the L2PcInstance with new LEVEL, MAX_HP and MAX_MP</li>
	 ...
	 * to inform (exclusive broadcast)</li> <li>Recalculate the party level</li>
	 * <li>Recalculate the number of Recommandation that the L2PcInstance can
	 * give</li> <li>Give Expertise skill of this level and remove beginner
	 * Lucky skill</li><BR>
	 * <BR>
	 * 
	 */
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Set the RegenActive flag to False</li> <li>Stop the HP/MP/CP
	 * Regeneration task</li><BR>
	 * <BR>
	 * 
	 */
	...
	public void onTradeStart(L2PcInstance partner) {
		...
		SystemMessage msg = new SystemMessage(
				SystemMessageId.BEGIN_TRADE_WITH_S1);
	...
	public void onTradeConfirm(L2PcInstance partner) {
		SystemMessage msg = new SystemMessage(
				SystemMessageId.S1_CONFIRMED_TRADE);
		msg.addString(partner.getName());
		sendPacket(msg);
	}
	...
	public boolean disarmWeapons() {
		...
		// Unequip the weapon
		L2ItemInstance wpn = getInventory().getPaperdollItem(
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Set the Intention of the AI to AI_INTENTION_IDLE</li> <li>Enable all
	 * skills (set _allSkillsDisabled to False)</li> <li>Send a Server->Client
	 * Packet MagicSkillCanceld to the L2PcInstance and all L2PcInstance in the
	...
	private static L2PcInstance restore(int objectId) {
		...
		try {
			...
			while (rset.next()) {
				...
				PcAppearance app = new PcAppearance(rset.getByte("face"), rset
						.getByte("hairColor"), rset.getByte("hairStyle"),
						female);

				player = new L2PcInstance(objectId, template, rset
						.getString("account_name"), app);
				...
				if (player.getClan() != null) {
					if (player.getClan().getLeaderId() != player.getObjectId()) {
						...
						player.setClanPrivileges(player.getClan().getRankPrivs(
				...
				// Check recs
				player.checkRecom(rset.getInt("rec_have"), rset
						.getInt("rec_left"));
				...
				player
						.setIsIn7sDungeon((rset.getInt("isin7sdungeon") == 1) ? true
								: false);
				...
				player.setAllianceWithVarkaKetra(rset
						.getInt("varka_ketra_ally"));
				...
				// invisible
				player.setXYZInvisible(rset.getInt("x"), rset.getInt("y"), rset
						.getInt("z"));

				// Retrieve the name and ID of the other characters assigned to
				// this account.
				PreparedStatement stmt = con
						.prepareStatement("SELECT obj_Id, char_name FROM characters WHERE account_name=? AND obj_Id<>?");
		...
		if (_forumMail == null) {
			...
			if (_forumMail == null) {
				ForumsBBSManager.getInstance().createNewForum(
						...
						getObjectId());
				setMail(ForumsBBSManager.getInstance().getForumByName(
						"MailRoot").getChildByName(getName()));
			}
		}
		...
		if (_forumMemo == null) {
			...
			if (_forumMemo == null) {
				ForumsBBSManager.getInstance().createNewForum(
						...
						getObjectId());
				setMemo(ForumsBBSManager.getInstance().getForumByName(
						"MemoRoot").getChildByName(_accountName));
			}
		}
	...
	private void storeRecipeBook() {
		...
		if (isSubClassActive())
			return;
		if (getCommonRecipeBook().length == 0
		...
		try {
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con
					.prepareStatement("DELETE FROM character_recipebook WHERE char_id=?");
			statement.setInt(1, getObjectId());
			statement.execute();
			statement.close();
	...
	private void restoreRecipeBook() {
		...
		try {
			...
			L2RecipeList recipe;
			while (rset.next()) {
				recipe = RecipeController.getInstance().getRecipeList(
						rset.getInt("id") - 1);
			...
			for (L2Effect effect : getAllEffects()) {
				if (effect != null && effect.getInUse()
					...
					if (ReuseTimeStamps.containsKey(skillId)) {
						...
						statement.setLong(6, t.hasNotPassed() ? t.getReuse()
								: 0);
					} else {
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Replace oldSkill by newSkill or Add the newSkill</li> <li>If an old
	 * skill has been replaced, remove all its Func objects of L2Character
	 * calculator set</li> <li>Add Func objects of newSkill to the calculator
	 * set of the L2Character</li><BR>
	 * <BR>
	 * 
	 * @param newSkill
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Remove the skill from the L2Character _skills</li> <li>Remove all its
	 * Func objects from the L2Character calculator set</li><BR>
	 * <BR>
	 * 
	...
	public L2Skill removeSkill(L2Skill skill) {
		...
		for (L2ShortCut sc : allShortCuts) {
			if (sc != null && skill != null && sc.getId() == skill.getId()
	...
	public void checkAllowedSkills()
	{
		...
		skill_loop: for (L2Skill skill : getAllSkills())
		{
			...
			_log.warning("Cheater! - Character " + getName() + " of Account " + getAccountName() + " donator status :" + isDonator() + " got skill " + skill.getName() + " removed!");
		}
	}
	...
	private void restoreSkills() {
		...
		try {
			...
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con
					.prepareStatement(Config.ALT_SUBCLASS_SKILLS ? RESTORE_SKILLS_FOR_CHAR_ALT_SUBCLASS
	...
	private void restoreHenna() {
		...
		try {
			...
			while (rset.next()) {
				...
				if (symbol_id != 0) {
					L2Henna tpl = HennaTable.getInstance().getTemplate(
							symbol_id);
	...
	public void setChatBanned(boolean isBanned) {
		...
		if (isChatBanned()) {
			...
			if (_banchat_timer > 0)
				_BanChatTask = ThreadPoolManager.getInstance().scheduleGeneral(
						new SchedChatUnban(this), _banchat_timer);
		} else {
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Check if the attacker isn't the L2PcInstance Pet</li> <li>Check if
	 * the attacker is L2MonsterInstance</li> <li>If the attacker is a
	 * L2PcInstaknce, check if it is not in the same siege clan (Attacker,
	 * Defender)</li><BR>
	 * <BR>
	 * <li>Check if the L2PcInstance has Karma</li> <li>If the attacker is a
	 * L2PcInstance, check if it is not in the same siege clan (Attacker,
	 * Defender)</li><BR>
	 * <BR>
	 * 
	 */
	...
	public boolean isAutoAttackable(L2Character attacker) 
	{
		...
		if ((attacker instanceof L2PcInstance)
				&& ((L2PcInstance) attacker).isgood())
			return true;

		if ((attacker instanceof L2PcInstance)
				&& ((L2PcInstance) attacker).isevil())
			return true;
		...
		if (attacker instanceof L2PcInstance) {
			...
			if (isInsideZone(ZONE_PVP)
					&& ((L2PcInstance) attacker).isInsideZone(ZONE_PVP))
				return true;
			...
			if (getClan() != null) {
				...
				if (siege != null) {
					...
					if (siege.checkIsDefender(((L2PcInstance) attacker)
							.getClan())
					...
					if (siege.checkIsAttacker(((L2PcInstance) attacker)
							.getClan())
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Check if the skill isn't toggle and is offensive</li> <li>Check if
	 * the target is in the skill cast range</li> <li>Check if the skill is
	 * Spoil type and if the target isn't already spoiled</li> <li>Check if the
	 * caster owns enought consummed Item, enough HP and MP to cast the skill</li>
	 * <li>Check if the caster isn't sitting</li> <li>Check if all skills are
	...
	public void useMagic(L2Skill skill, boolean forceUse, boolean dontMove) {
		...
		{
			sendPacket(new SystemMessage(
					SystemMessageId.OBSERVERS_CANNOT_PARTICIPATE));
			abortCast();
			sendPacket(new ActionFailed());
			return;
		}
		...
		if (sklTargetType == L2SkillTargetType.TARGET_SIGNET_GROUND
				&& worldPosition == null) {
			_log.info("WorldPosition is null for skill: " + skill.getName()
					+ ", player: " + getName() + ".");
			sendPacket(new ActionFailed());
			return;
		}
		...
		if (isInDuel()) {
			if (!(target instanceof L2PcInstance && ((L2PcInstance) target)
					.getDuelId() == getDuelId())) {
		...
		if (_inEventTvT && TvT._started && !Config.TVT_ALLOW_ENEMY_HEALING)
		{
			...
			{
				if (!isMemberOfSameTeam(((L2PcInstance) this), (L2PcInstance) target))
				{
				...
				{
					if (target instanceof L2PcInstance && skill.getTargetType() != L2SkillTargetType.TARGET_SELF
		...
		if (isSkillDisabled(skill.getId())
			...
			SystemMessage sm = new SystemMessage(
					SystemMessageId.SKILL_NOT_AVAILABLE);
		...
		if (_inEventCTF && !Config.CTF_ALLOW_SUMMON && CTF._started && sklType == L2SkillType.SUMMON)
		{
			sendPacket(new SystemMessage(SystemMessageId.NOTHING_HAPPENED));
		}
		...
		if (skill.getItemConsume() > 0) {
			...
			L2ItemInstance requiredItems = getInventory().getItemByItemId(
		...
		if (isAlikeDead() && !skill.isPotion()
				&& skill.getSkillType() != L2SkillType.FAKE_DEATH) {
			// Send a Server->Client packet ActionFailed to the L2PcInstance
			sendPacket(new ActionFailed());
			return;
		}
		...
		if (skill.isOffensive()) {
			if ((isInsidePeaceZone(this, target))
				...
				// ActionFailed
				sendPacket(new SystemMessage(
						SystemMessageId.TARGET_IN_PEACEZONE));
				sendPacket(new ActionFailed());
				return;
			}
			...
			if (dontMove) {
					...
					if (!isInsideRadius(worldPosition.getX(), worldPosition.getY(), worldPosition.getZ(), skill.getCastRange() + getTemplate().collisionRadius, false, false)) 
				{
						// Send a System Message to the caster
						sendPacket(new SystemMessage(
								SystemMessageId.TARGET_TOO_FAR));
						...
						&& !isInsideRadius(target, skill.getCastRange()
								+ getTemplate().collisionRadius, false, false)) {
					// Send a System Message to the caster
		...
		if (!skill.isOffensive() && target instanceof L2MonsterInstance
				&& !forceUse) {
		...
		if (sklType == L2SkillType.SWEEP && target instanceof L2Attackable) {
			...
			if (((L2Attackable) target).isDead()) {
				if (!((L2Attackable) target).isSpoil()) {
					// Send a System Message to the L2PcInstance
					sendPacket(new SystemMessage(
							SystemMessageId.SWEEPER_FAILED_TARGET_NOT_SPOILED));
				...
				if (getObjectId() != spoilerId && !isInLooterParty(spoilerId)) {
					// Send a System Message to the L2PcInstance
					sendPacket(new SystemMessage(
							SystemMessageId.SWEEP_NOT_ALLOWED));
	...
	public boolean isInLooterParty(int LooterId) {
		L2PcInstance looter = (L2PcInstance) L2World.getInstance().findObject(
				LooterId);
		...
		if (target != null
			...
			if (skill.isPvpSkill()) // pvp skill
			{
				if (getClan() != null
					...
					if (getClan().isAtWarWith(
							((L2PcInstance) target).getClan().getClanId())
							&& ((L2PcInstance) target).getClan().isAtWarWith(
									getClan().getClanId()))
	...
	public boolean setMountType(int mountType) {
		...
		case 1:
			setIsRiding(true);
			if (isNoble()) {
				L2Skill striderAssaultSkill = SkillTable.getInstance().getInfo(
						325, 1);
				...
				if (magic) {
					if (!summon) {
						if (itemId == 2509 || itemId == 2510 || itemId == 2511
							...
							handler = ItemHandler.getInstance().getItemHandler(
									itemId);
	...
	class WarnUserTakeBreak implements Runnable {
		public void run() {
			if (L2PcInstance.this.isOnline() == 1) {
				SystemMessage msg = new SystemMessage(
						SystemMessageId.PLAYING_FOR_LONG_TIME);
	...
	class WaterTask implements Runnable {
		public void run() {
			...
			SystemMessage sm = new SystemMessage(
					SystemMessageId.DROWN_DAMAGE_S1);
	...
	public void setHero(boolean hero) {
		...
		} else if (getCount() >= Config.HERO_COUNT && hero
				&& Config.ALLOW_HERO_SUBSKILL) {
			for (L2Skill s : HeroSkillTable.GetHeroSkills())
				addSkill(s, false); // Dont Save Hero skills to database
		} else {
			for (L2Skill s : HeroSkillTable.GetHeroSkills())
	...
	// L2l2jhellas Faction Good vs Evil
	...
	public boolean addSubClass(int classId, int classIndex)
	{
		...
		for (L2SkillLearn skillInfo : skillTree) {
			if (skillInfo.getMinLevel() <= 40) {
				...
				L2Skill newSkill = SkillTable.getInstance().getInfo(
						skillInfo.getId(), skillInfo.getLevel());
	...
	public void stopRentPet() {
		if (_taskRentPet != null) {
			...
			if (setMountType(0)) // this should always be true now, since we
									// teleported already
			{
	...
	public void onPlayerEnter() {
		...
		if (SevenSigns.getInstance().isSealValidationPeriod()
			...
			if (!isGM()
					&& isIn7sDungeon()
					&& SevenSigns.getInstance().getPlayerCabal(this) != SevenSigns
	...
	public void reviveRequest(L2PcInstance Reviver, L2Skill skill, boolean Pet) {
		if (_reviveRequested == 1) {
			if (_revivePet == Pet) {
				Reviver.sendPacket(new SystemMessage(
				...
				if (Pet)
					Reviver.sendPacket(new SystemMessage(
							SystemMessageId.PET_CANNOT_RES)); // A pet cannot be
																// resurrected
																// while it's
																// owner is in
																// the process
					...
					Reviver.sendPacket(new SystemMessage(
							SystemMessageId.MASTER_CANNOT_RES)); // While a pet
																	// is
																	// attempting
																	// to
																	// resurrect,
			...
			if (isPhoenixBlessed())
				...
				_revivePower = Formulas.getInstance()
						.calculateSkillResurrectRestorePercent(
								skill.getPower(), Reviver.getWIT());

			_revivePet = Pet;
			sendPacket(new ConfirmDlg(SystemMessageId.RESSURECTION_REQUEST
					.getId(), Reviver.getName()));
		}
	}
	...
	public void reviveAnswer(int answer) {
		...
		// If character refuse a PhoenixBlessed autoress, cancel all buffs he
		// had
	...
	public final void onTeleported() {
		...
		if (getTrainedBeast() != null) {
			...
			getTrainedBeast().teleToLocation(
					getPosition().getX() + Rnd.get(-100, 100),
	 ...
	 * position before entering in observer mode</li> <li>Set the online Flag to
	 * True or False and update the characters table of the database with online
	 * status and lastAccess</li> <li>Stop the HP/MP/CP Regeneration task</li>
	 * <li>Cancel Crafting, Attak or Cast</li> <li>Remove the L2PcInstance from
	 * the world</li> <li>Stop Party and Unsummon Pet</li> <li>Update database
	 * with items in its inventory and remove them from the world</li> <li>
	 * Remove all L2Object from _knownObjects and _knownPlayer of the
	 * L2Character then cancel Attak or Cast and notify AI</li> <li>Close the
	...
	public void startFishing(int _x, int _y, int _z) {
		...
		List<FishData> fishs = FishTable.getInstance()
				.getfish(lvl, type, group);
		...
		fishs = null;
		sendPacket(new SystemMessage(
				SystemMessageId.CAST_LINE_AND_START_FISHING));
		...
		// in fishing.
		efs = new ExFishingStart(this, _fish.getType(), _x, _y, _z, _lure
				.isNightLure());
		broadcastPacket(efs);
		...
		if (!isDead() && _taskforfish == null) {
			...
			_taskforfish = ThreadPoolManager.getInstance()
					.scheduleEffectAtFixedRate(
							new LookingForFishTask(_fish.getWaitTime(), _fish
									.getFishGuts(), _fish.getType(), isNoob,
	...
	public void EndFishing(boolean win) {
		...
		if (_fishCombat == null)
			sendPacket(new SystemMessage(
					SystemMessageId.BAIT_LOST_FISH_GOT_AWAY));
		_fishCombat = null;
		_lure = null;
		// Ends fishing
	...
	public void setQueuedSkill(L2Skill queuedSkill, boolean ctrlPressed,
		...
		if (Config.DEBUG)
			_log.info("Setting queued skill: " + queuedSkill.getName()
					+ " (ID: " + queuedSkill.getId() + ") for " + getName()
					+ ".");
	...
	public void setInJail(boolean state, int delayInMinutes) {
		...
		if (_inJail) {
			if (delayInMinutes > 0) {
				...
				// start the countdown
				_jailTask = ThreadPoolManager.getInstance().scheduleGeneral(
						new JailTask(this), _jailTimer);
				sendMessage("You are in jail for " + delayInMinutes
						+ " minutes.");
			}

			// Open a Html message to inform the player
			NpcHtmlMessage htmlMsg = new NpcHtmlMessage(0);
			String jailInfos = HtmCache.getInstance().getHtm(
	...
	public void increaseDeathPenaltyBuffLevel() {
		...
		if (getDeathPenaltyBuffLevel() != 0) {
			L2Skill skill = SkillTable.getInstance().getInfo(5076,
		...
		addSkill(SkillTable.getInstance().getInfo(5076,
	 ...
	 * "Your chat ban has been lifted." on every login into game :)
	 */
	...
	public void setPunishLevel(PunishLevel state, int delayInMinutes) {
		...
		case CHAT: // Chat Ban
		{
			...
			if (delayInMinutes > 0) {
				...
				// start the countdown
				_punishTask = ThreadPoolManager.getInstance().scheduleGeneral(
						new PunishTask(this), _punishTimer);
				sendMessage("You are chat banned for " + delayInMinutes
						+ " minutes.");
			} else
	...
	private void updatePunishState()// TODO Next clean up this shood be active
	{
		if (getPunishLevel() != PunishLevel.NONE) {
			...
			if (_punishTimer > 0) {
				_punishTask = ThreadPoolManager.getInstance().scheduleGeneral(
						new PunishTask(this), _punishTimer);
				sendMessage("You are still " + getPunishLevel().string()
						+ " for " + Math.round(_punishTimer / 60000)
						+ " minutes.");
			}
	...
	public void updateTitle() {
		this.setTitle(Config.PVP_TITLE_PREFIX + getPvpKills()
				+ Config.PK_TITLE_PREFIX + getPkKills() + " ");
	}
