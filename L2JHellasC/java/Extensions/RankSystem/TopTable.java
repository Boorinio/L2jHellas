...
public class TopTable
{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			for (int i = 1; i <= TOP_LIMIT; i++)
			{
				...
				if (bestKiller != 0)
				{
					...
					// get character data:
					statement = con.prepareStatement("SELECT char_name as name, base_class as base_class, level as level FROM characters WHERE " + RankLoader.CHAR_ID_COLUMN_NAME + " = ?");
	...
	private void restoreTopTable()
	{
		...
		{
			// get top killers:
			statement = con.prepareStatement("SELECT position, player_id, value, table_id, char_name as name, base_class as base_class, level as level FROM rank_pvp_system_top_table JOIN characters ON rank_pvp_system_top_table.player_id = characters." + RankLoader.CHAR_ID_COLUMN_NAME + " ORDER BY position");
			rset = statement.executeQuery();
