...
public class Potions implements IItemHandler
{
	...
	public synchronized void useItem(L2Playable playable, L2ItemInstance item)
	{
		...
		// MANA POTIONS
			...
			case 8627:
				...
				{
					SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.INCOMPATIBLE_ITEM_GRADE); // INCOMPATIBLE_ITEM_GRADE
	...
	private static boolean isEffectReplaceable(L2PcInstance activeChar, Enum<?> effectType, int itemId)
	{
		...
		for (L2Effect e : effects)
		{
			if (e.getEffectType() == effectType)
			{
				...
				// It would be faster to check if its > 10 but that would screw
