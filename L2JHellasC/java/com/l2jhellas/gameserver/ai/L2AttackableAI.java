...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target instanceof L2PcInstance)
		{
			...
			if ((!(me instanceof L2RaidBossInstance) || !(me instanceof L2GuardInstance)) && ((L2PcInstance) target).isSilentMoving())
				return false;
			...
			if (!(me instanceof L2GuardInstance) && ((L2PcInstance) target).isRecentFakeDeath())
				return false;
	...
	private void thinkActive()
	{
		...
		else if (npc.getSpawn() != null && Rnd.nextInt(RANDOM_WALK_RATE) == 0)
		{
			...
			if (npc.getSpawn().getLocx() == 0 && npc.getSpawn().getLocy() == 0)
			{
				...
				if (Territory.getInstance().getProcMax(npc.getSpawn().getLocation()) > 0)
					return;
